...
for (const scope of campaign.places) {
  ...
  const directory = path.join(root, "us", "ca", slug(scope.city), "parks", slug(scope.name));
  ...
  need((sourcePlace.searchAnswers || []).length >= 12, `${scope.name}: official parent answer set was lost`);
  need(place.address?.includes(", CA "), `${scope.name}: full California address missing`);
  if (selections.researchQueue[scope.id]) need((place.researchQueue || []).includes(selections.researchQueue[scope.id]), `${scope.name}: unresolved photo queue missing`);
...
need(featureCount === 11, `Expected 11 evidence-complete subsites, found ${featureCount}`);
const joined = campaign.places.map((scope) => JSON.stringify(ready.find((place) => place.id === scope.id) || {})).join("\n");
for (const phrase of [
  "Penitencia Creek Trail is closed at the Trestle Bridge",
  "The trail opens one hour before sunrise and closes one hour after sunset",
  "For 2026, grounds are listed daily 9:00 a.m.-4:00 p.m.",
  "Fairytale Town is separately ticketed",
  "Cell service is weak or absent",
  "The central fountain is ornamental, not a splash pad"
]) need(joined.includes(phrase), `Missing practical guidance: ${phrase}`);

const sitemap = fs.readFileSync(path.join(root, "sitemap.xml"), "utf8");
for (const scope of campaign.places) {
  const route = `/us/ca/${slug(scope.city)}/parks/${slug(scope.name)}`;
...
console.log("Verified 9 Northern California guides, 36 licensed photos, 11 evidence-complete subsite pages, raw HTML, exact navigation, and sitemap routes.");
