...
const joined = campaign.places
  ...
  .join("\n");
for (const phrase of [
  "Texas Beach pedestrian bridge is closed while repairs continue toward a December 2026 target",
  "Pick the mansion and gardens, farm and wildlife, or Nature Center",
  "Pedal boats and concessions are seasonal",
  "Wet trails can be damaged by use after heavy rain",
  "small neighborhood overlook rather than a full-service regional park",
  "planned event reopening October 9-11, 2026",
  "General admission is free, but major special exhibitions require",
  "Fort Harrison Visitor Center is closed throughout 2026",
])
  need(joined.includes(phrase), `Missing Richmond guidance: ${phrase}`);
...
need(app.includes("place.temporarilyClosed === true"), "Temporary-closure status support missing");
if (fail.length) {
  console.error(fail.join("\n"));
  process.exit(1);
}
console.log(
  `Verified ${campaign.places.length} Richmond guides with four photos, eight mapped subsites and practical visitor answers.`,
);
