function hasDocumentedReuseRights(image) {
  ...
  return /\b(public[ -]domain|pdm(?:\s+1\.0)?|cc0|cc[ -]?by(?:[ -]?sa)?|by(?:-sa)?(?:\s+\d)|creative commons|free art license|fal|permission(?:-| )?(?:cleared|granted)|used with permission)\b/i.test(license);
}
