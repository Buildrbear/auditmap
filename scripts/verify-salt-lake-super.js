...
for (const scope of campaign.places) {
  ...
  for (const f of p.features || []) {
    ...
    const dx = (f.longitude - p.longitude) * 82;
    need(Math.hypot(dx, dy) < 25, `${scope.name}/${f.name}: implausibly distant coordinate`);
    ...
    if (fs.existsSync(featureFile)) {
      ...
      need(html.includes(encodeURIComponent(`${f.latitude},${f.longitude}`).toLowerCase()), `${scope.name}/${f.name}: exact-coordinate navigation missing`);
    }
  }
}
const joined = campaign.places
  ...
  .join("\n");
for (const phrase of [
  "grand opening announced for September 2026",
  "Big Field remains open and continues to host permitted events",
  "Private vehicles are currently prohibited in City Creek Canyon",
  "upper gate closes November 1",
  "No pets are allowed anywhere in Big Cottonwood Canyon",
  "No pets are allowed anywhere in Little Cottonwood Canyon",
  "main Visitor Center is closed for renovation",
  "Lake levels are low and shoreline conditions change",
])
  need(joined.includes(phrase), `Missing Salt Lake City and Wasatch Front guidance: ${phrase}`);
...
console.log(
  `Verified ${campaign.places.length} Salt Lake City and Wasatch Front guides with four photos, eight mapped subsites and practical visitor answers.`,
);
