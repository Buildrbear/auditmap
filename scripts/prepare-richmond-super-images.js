...
const fs = require("node:fs"),
  ...
  research = require("../data/richmond-photo-research.json"),
...
async function bytes(url, label) {
  for (let n = 0; n < 6; n++) {
    const official =
      ...
      url.includes("venturerichmond.com") ||
...
(async () => {
  ...
  for (const [id, e] of Object.entries(research.places)) {
    const name = names.get(id),
      dir = path.join(root, "assets/parks/richmond-super", slug(name));
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/richmond-super-images.json"),
