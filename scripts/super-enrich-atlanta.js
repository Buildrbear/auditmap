...
const crypto = require("node:crypto"),
  ...
  campaign = require("../data/atlanta-super-enrichment-campaign.json"),
  facts = require("../data/atlanta-visitor-facts.json").places,
  galleries = require("../data/generated/atlanta-super-images.json"),
  coordinates =
    require("../data/generated/atlanta-feature-coordinates.json").places,
  profiles = require("../data/atlanta-feature-profiles.json"),
  profileAdditions = require("../data/atlanta-feature-profile-additions.json"),
...
const slug = (v) =>
  ...
  everyDay = (o, c) => Object.fromEntries(days.map((d) => [d, [[o, c]]])),
  schedules = Object.fromEntries(
    campaign.places.map((p) => [
      p.id,
      everyDay(p.id.includes("centennial") ? "07:00" : "06:00", "23:00"),
    ]),
  );
const stable = (parent, s) => {
    const b = crypto
      .createHash("sha256")
...
function answers(p) {
  return [
    ...
    [
      ...
      `Check Atlanta's hourly forecast, heat index, rain, lightning, air quality, and sunset. Summer heat and thunderstorms can make exposed lawns, paved trails, splash pads, pools, and wooded creek routes unsafe or unavailable.`,
    ],
  ].map((v) => ans(p, ...v));
}
function feature(p, name, i, images) {
  const s = slug(name),
    id = stable(p.id, s),
    profile = profiles[p.id]?.[i] || profileAdditions[name],
    point = coordinates[p.id]?.[s];
  if (!profile || !point)
    throw new Error(`${p.name}/${name}: profile or coordinate missing`);
  const image = {
    ...
    separateHours =
      /aquatic|pool|fountain|splash|recreation center|visitor center|amphitheater|golf|zoo|farmers market|arts center|horse park/i.test(
        name,
      ),
    featureHours = separateHours
      ? `${name} follows its own program, event, seasonal, or staffed-facility schedule. Confirm today's availability with ${p.operator} before making a special trip.`
      : p.hours,
    qs = [
  ...
  return {
    ...
    details: {
      ...
      address: p.address,
      hours: featureHours,
      hoursSchedule: separateHours ? false : schedules[p.id],
      cost: p.cost,
...
(() => {
  ...
  for (const scope of campaign.places) {
    ...
    const record = {
      ...
      summary: p.summary,
      searchDescription: `Hours, arrival guidance, real photos, mapped destinations, and essential visitor answers for ${p.name} in Atlanta.`,
      address: p.address,
      latitude: p.latitude,
      longitude: p.longitude,
      hours: p.hours,
      hoursSchedule: schedules[p.id],
      cost: p.cost,
      accessibility: p.accessibility,
      sourceLabel: p.operator,
      source: p.source,
  ...
  console.log(`Super-enriched ${campaign.places.length} Atlanta guides.`);
})();
