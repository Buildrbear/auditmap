...
const fs = require("node:fs"),
  ...
  facts = require("../data/kansas-city-visitor-facts.json").places,
  galleries = require("../data/generated/kansas-city-super-images.json"),
  coordinates =
    require("../data/generated/kansas-city-feature-coordinates.json").places,
...
function schedule(p) {
  if (p.state === "MO" && !p.id.endsWith("berkley-riverfront-park"))
    return daily("05:00", "23:59");
  return false;
}
...
function answers(p) {
  return [
    ...
    [
      ...
      `Check current Kansas City metro weather and operator alerts. Missouri River conditions, flash flooding, lake status, heat, severe storms, snow, ice and high winds can close trails or facilities independently.`,
    ],
  ].map((v) => ans(p, ...v));
}
...
function featureImageIndex(p, name, i) {
  const n = name.toLowerCase();
  if (p.id.endsWith("jacob-l-loose-park")) { if (n.includes("rose")) return 1; if (n.includes("pond")) return 2; if (n.includes("japanese")) return 3; return i % 4; }
  if (p.id.endsWith("berkley-riverfront-park")) { if (n.includes("streetcar")) return 3; if (n.includes("trail")||n.includes("bridge")) return 0; return i % 4; }
  if (p.id.endsWith("shawnee-mission-park")) { if (n.includes("dog")) return 3; if (n.includes("marina")||n.includes("lake")||n.includes("beach")) return 2; return i % 4; }
  return i;
}
function feature(p, name, i, images) {
  ...
  const base = images[featureImageIndex(p, name, i) % images.length],
    ...
    separateHours = /(zoo|aquarium|theatre|nature center|dog park|off leash|sprayground|playground|garden|tennis|museum|tower|skate|fountain|streetcar|stadium|volleyball|beach|marina|disc golf|pump track|visitor center|café|library|shelter)/i.test(name),
    specificHours = name === "Shawnee Mission Park Beach"
      ? "The 2026 beach season is May 23-August 9: noon-4:00 p.m. Monday, Wednesday and Friday; noon-5:00 p.m. Tuesday and Thursday; and noon-6:00 p.m. Saturday and Sunday. It opens at 1:00 p.m. August 2, closes August 5, and opens at 1:00 p.m. August 8-9. Weather, water conditions and staffing can change the schedule."
      : name === "Loose Park Sprayground"
        ? "The 2026 sprayground season is May 23-September 7, daily 11:00 a.m.-7:00 p.m., subject to weather, maintenance and same-day operator notices."
        : "",
    featureHours = specificHours || (separateHours
      ? `${name} keeps its own admission, operating, seasonal, staffing, event, maintenance, transit, or weather schedule. Check the cited official source and current operator notices before leaving.`
      : p.hours),
    qs = [
...
(() => {
  ...
  console.log(`Super-enriched ${campaign.places.length} Kansas City guides.`);
})();
