...
const fs = require("node:fs"),
  ...
  research = require("../data/las-vegas-photo-research.json"),
...
async function bytes(url, label) {
  for (let n = 0; n < 6; n++) {
    const official =
      url.includes("lasvegasnevada.gov") ||
      url.includes("clarkcountynv.gov") ||
      url.includes("springspreserve.org") ||
      url.includes("blm.gov") ||
      url.includes("parks.nv.gov") ||
      url.includes("nps.gov"),
      r = await fetch(url, {
        headers: {
          "User-Agent": official
...
(async () => {
  ...
  for (const [id, e] of Object.entries(research.places)) {
    const name = names.get(id),
      dir = path.join(root, "assets/parks/las-vegas-super", slug(name));
    ...
    for (const [candidate, index] of e.candidates.map((x, i) => [x, i])) {
      ...
      images.push({
        ...
        author: rec.author,
        license: rec.license,
        alt: `${name} in the Las Vegas and Southern Nevada area`,
        width: m.width,
        height: m.height,
      });
    }
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/las-vegas-super-images.json"),
