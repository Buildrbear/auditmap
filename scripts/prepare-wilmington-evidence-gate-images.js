...
const selections = require("../data/wilmington-evidence-gate-photo-selections.json");
...
(async () => {
  ...
  for (const [id, entry] of Object.entries(selections.places)) {
    ...
    const directory = path.join(root, "assets/parks/wilmington-evidence-gate", slug(name));
  ...
  fs.writeFileSync(path.join(root, "data/generated/wilmington-evidence-gate-images.json"), `${JSON.stringify(output, null, 2)}\n`);
