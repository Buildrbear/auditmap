...
async function locate(name, parent, base) {
  for (const q of [`${name}, ${parent}, Indiana`, `${name}, Indianapolis, Indiana`]) {
    const r = await fetch(
...
(async () => {
  ...
  for (const p of campaign.places) {
    ...
    for (const [i, name] of p.subsites.entries()) {
      ...
      if (!hit)
        try {
          hit = await locate(name, p.name, base);
        } catch (e) {
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/indianapolis-feature-coordinates.json"),
