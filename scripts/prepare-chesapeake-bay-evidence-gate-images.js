...
const selections = require("../data/chesapeake-bay-evidence-gate-photo-selections.json");
...
(async () => {
  ...
  for (const [id, entry] of Object.entries(selections.places)) {
    ...
    const directory = path.join(root, "assets/parks/chesapeake-bay-evidence-gate", slug(name));
  ...
  fs.writeFileSync(path.join(root, "data/generated/chesapeake-bay-evidence-gate-images.json"), `${JSON.stringify(output, null, 2)}\n`);
