...
const fs = require("node:fs"),
  ...
  overrides = require("../data/hampton-roads-coordinate-overrides.json"),
...
(async () => {
  ...
  for (const p of campaign.places) {
    ...
    for (const [i, name] of p.subsites.entries()) {
      ...
      let hit = old
        ...
        : null;
      const override = overrides[p.id]?.[key];
      if (override)
        hit = {
          latitude: override[0],
          longitude: override[1],
          displayName: `Reviewed placement for ${name}`,
          source: "AuditMap reviewed public-map placement",
        };
      if (!hit)
        try {
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/hampton-roads-feature-coordinates.json"),
