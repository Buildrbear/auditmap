...
const fs = require("node:fs"),
  ...
  research = require("../data/hampton-roads-photo-research.json"),
...
async function bytes(url, label) {
  for (let n = 0; n < 6; n++) {
    const official =
      url.includes("rva.gov") ||
      url.includes("maymont.org") ||
      url.includes("vmfa.museum") ||
      url.includes("venturehampton-roads.com") ||
...
(async () => {
  ...
  for (const [id, e] of Object.entries(research.places)) {
    const name = names.get(id),
      dir = path.join(root, "assets/parks/hampton-roads-super", slug(name));
    ...
    for (const [candidate, index] of e.candidates.map((x, i) => [x, i])) {
      ...
      images.push({
        ...
        author: rec.author,
        license: rec.license,
        alt: `${name} in the Hampton Roads area`,
        width: m.width,
        height: m.height,
      });
    }
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/hampton-roads-super-images.json"),
