...
const fs = require("node:fs"),
  ...
  research = require("../data/salt-lake-photo-research.json"),
...
async function bytes(url, label) {
  for (let n = 0; n < 6; n++) {
    const official =
      url.includes("slc.gov") ||
      url.includes("sugarhousepark.org") ||
      url.includes("fs.usda.gov") ||
      url.includes("stateparks.utah.gov"),
      r = await fetch(url, {
        headers: {
          "User-Agent": official
...
(async () => {
  ...
  for (const [id, e] of Object.entries(research.places)) {
    const name = names.get(id),
      dir = path.join(root, "assets/parks/salt-lake-super", slug(name));
    ...
    for (const [candidate, index] of e.candidates.map((x, i) => [x, i])) {
      ...
      images.push({
        ...
        author: rec.author,
        license: rec.license,
        alt: `${name} in the Salt Lake City and Wasatch Front area`,
        width: m.width,
        height: m.height,
      });
    }
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/salt-lake-super-images.json"),
