...
const fs = require("node:fs"),
  ...
  research = require("../data/honolulu-photo-research.json"),
...
async function bytes(url, label) {
  for (let n = 0; n < 6; n++) {
    const official =
      url.includes("honolulu.gov") ||
      url.includes("hawaii.gov") ||
...
(async () => {
  ...
  for (const [id, e] of Object.entries(research.places)) {
    const name = names.get(id),
      dir = path.join(root, "assets/parks/honolulu-super", slug(name));
    ...
    for (const [candidate, index] of e.candidates.map((x, i) => [x, i])) {
      ...
      images.push({
        ...
        author: rec.author,
        license: rec.license,
        alt: `${name} in the Honolulu and Oahu area`,
        width: m.width,
        height: m.height,
      });
    }
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/honolulu-super-images.json"),
