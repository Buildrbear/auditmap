...
for (const scope of campaign.places) {
  ...
  for (const f of p.features || []) {
    ...
    const dx = (f.longitude - p.longitude) * 91;
    need(Math.hypot(dx, dy) < 10, `${scope.name}/${f.name}: implausibly distant coordinate`);
...
const joined = campaign.places
  ...
  .join("\n");
for (const phrase of [
  "reservation alone does not pay admission",
  "extreme-heat advisories can remove major outdoor experiences",
  "Wetlands Loop does not currently complete a full circuit",
  "protected dunes are a historic habitat area",
  "entrance kiosk no longer accepts cash",
  "no water, shade or toilets",
  "ordinary pool toys are prohibited",
  "West Entrance is closed Monday-Thursday",
])
  need(joined.includes(phrase), `Missing Las Vegas and Southern Nevada guidance: ${phrase}`);
...
console.log(
  `Verified ${campaign.places.length} Las Vegas and Southern Nevada guides with four photos, eight mapped subsites and practical visitor answers.`,
);
