...
const fs = require("node:fs"),
  ...
  research = require("../data/tampa-bay-photo-research.json"),
...
async function bytes(url, label) {
  for (let n = 0; n < 6; n++) {
    const official =
      url.includes("tampa.gov") ||
      url.includes("hcfl.gov") ||
      url.includes("hillsboroughcounty") ||
      url.includes("stpeteparksrec.org") ||
      url.includes("stpetepier.org"),
      r = await fetch(url, {
        headers: {
          "User-Agent": official
...
(async () => {
  ...
  for (const [id, e] of Object.entries(research.places)) {
    const name = names.get(id),
      dir = path.join(root, "assets/parks/tampa-bay-super", slug(name));
    ...
    for (const [candidate, index] of e.candidates.map((x, i) => [x, i])) {
      ...
      images.push({
        ...
        author: rec.author,
        license: rec.license,
        alt: `${name} in the Tampa Bay area`,
        width: m.width,
        height: m.height,
      });
    }
  ...
  fs.writeFileSync(
    path.join(root, "data/generated/tampa-bay-super-images.json"),
